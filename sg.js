 ...
 SG constructor params, all passed on to create
  ...
  data: should be of the form 
	data = [ { 'id' : string,   //name for data row. used to find pairs to connect with slopelines
				...
				'val' : number 
					  : display value - to show in place of val?
			  } ]
...
(function() {
	var SG = window.SG = function(data, svgEl, settings) {
		...
		function setSettings(sg, settings) {
			...
			if(!isEmpty(settings.rowCurve)) {
				sg.rowCurve = settings.rowCurve;
			}
			sg.debugGrid = settings.debugGrid === true;
			if (!isEmpty(settings.doc)) {
				sg.doc = settings.doc;
			}
			if (!isNaN(settings.width)) {
				sg.width = settings.width;
			}
			if (!isNaN(settings.decimals)) {
				sg.decimals = settings.decimals;
			}
			if (!isEmpty(settings.lineColorHi)) {
				sg.lineColorHi = settings.lineColorHi;
			}
			if (!isEmpty(settings.lineColorLo)) {
				sg.lineColorLo = settings.lineColorLo;
			}
		...
		***/
		...
		function graph(sg) {
			if(sg.debugGrid) {
				drawGrid(sg.el, sg.rowh * 2,sg.rowh, sg.height, 800, sg.minv, (sg.sortVals == 'up' ? 1 : -1) * sg.forced, sg.offsetX, sg.offsetY, sg.rowCurve == 'log');
			}
		...
		***/
		...
		function repassGraphSet(curr, last, maxtw, width, height, gutter, strokeWidth) {
			...
			function fixTextWidth(d) {
				if (d.setcnt != this.setc - 1 && d.s != this.sorted.length - 1) {
					at(d.el, 'x', right(d.tw, maxtw, d.x));
				}
			}
